...
if(Math.random()<buffedZombieSpawnChance && zombieID !== 1  && miniBosses<miniBossLimit){
  console.log('hey')
  miniBosses ++;
...
else if(Math.random()<dumbZombieSpawnRate && zombieID !== 1){
  zombies[currentID][20] = setTimeout(() => {
    console.log('lol')
...
<div id="version-info"><div>Version 1.3.0</div><a href="patches.html" target="_blank"><button id="version-button">Patch Notes</button></a> </div>
`}
}
function credit(){
  const Credit = () =>{
...
function ShootTurret(tID,ShootFlag,tPID){
  ...
  let zombieTarget;
    ...
    <img id="TB${tPID}" src="object_frames/bullet.png" class="bullet">
    `;
