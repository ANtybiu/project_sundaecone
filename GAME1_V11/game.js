...
<div id="version-info"><div>Version 1.3.1</div><a href="patches.html" target="_blank"><button id="version-button">Patch Notes</button></a> </div>
`}
}
function credit(){
  const Credit = () =>{
...
Object.keys(walls).forEach((wallName) => {
  ...
  if (zX + intendedVelocityX < wallX + wallOBJ.width &&  zX + limit + intendedVelocityX > wallX && zY+intendedVelocityY < wallY + wallOBJ.height &&  zY + limit + intendedVelocityY > wallY) {
    ...
    let repulsionX = 1.5;
    let repulsionY = 1.5;
    if ((zX - wallX)<0){repulsionX = -1.5}else{repulsionX = 1.5}
    if ((zY - wallY)<0){repulsionY = -1.5}else{repulsionY = 1.5}
    zombie[0] += repulsionX 
    zombie[1] += repulsionY 
    wall[2] -= 0.025;
    if(!wall[3]){
